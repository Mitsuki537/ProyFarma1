...
{
    public class ClienteRepository<T> : IRepository<T> where T : class
    {
        private readonly HttpClient _httpClient;
        private readonly string _endpoint;

        public ClienteRepository(HttpClient httpClient, string endpoint)
        {
        ...
        public async Task<int> CreateAsync(object dto)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var createdEntity = JsonConvert.DeserializeObject<ClienteDto>(result);
                return createdEntity.IdCustomer;
            }

            var errorResponse = await response.Content.ReadAsStringAsync();
            throw new Exception($"Error al crear cliente: {errorResponse}");
        }
        ...
        public async Task<T> GetByIdAsync(int id)
        {
            ...
            throw new Exception($"Error al obtener cliente: {await response.Content.ReadAsStringAsync()}");
        }
